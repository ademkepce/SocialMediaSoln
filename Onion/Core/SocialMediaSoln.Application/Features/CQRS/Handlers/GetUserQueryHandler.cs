...
namespace SocialMediaSoln.Application.Features.CQRS.Handlers
{
    public class GetUserQueryHandler : IRequestHandler<GetUserQueryRequest, UserListDto>
    {
        ...
        {
            var data = await _repository.GetQueryable().Where(x => x.Id == request.Id).Include(x => x.Posts).ThenInclude(x => x.Likes).Include(x => x.Posts).ThenInclude(x => x.Comments).ThenInclude(x => x.AppUser).AsQueryable().Include(x => x.Followers).Include(x => x.Followings).FirstOrDefaultAsync();
